...
// }
//
// std::string decrypt(const std::string &encryptedBlock, const std::string key) {
...
//         key48[16 - i - 1] = "";
//         for (int j = 0; j < 48; j++)
//             key48[16 - i - 1] += keys56[i][pc_2[j] - 1]; // making 48 bits keys
//
//         std::cout << "Key48[" << i << "]: " << key48[16 - i - 1] << std::endl;
//     }
//
//     std::vector<std::string> hexTextBlocks;
//     for (int i = 0; i < encryptedBlock.length(); i += 16) {
//         hexTextBlocks.push_back(encryptedBlock.substr(i, 16));
//     }
//
//     std::string encrypted_bin = "";
//
...
//         L_32[15] = R;
//         R_32[15] = "";
//         R_32[15] = xor_add(P_R[15], L);
//         std::cout << "Encryption Round " << 1 << ":\n";
//         std::cout << "    R48: " << R48[15] << "\n";
//         std::cout << "    R_xor_K: " << R_xor_K[15] << "\n";
//         std::cout << "    P_R: " << P_R[15] << "\n";
//
//         for (int i = 1; i < 16; i++) {
//             L_32[15-i] = R_32[15-i - 1];
//             R48[15-i] = "";
//             for (int j = 0; j < 48; j++)
//                 R48[15-i] += R_32[15-i - 1][E_t[j] - 1];
//
//             R_xor_K[15-i] = xor_add(R48[15-i], key48[15-i]); // fill the R_xor_K
//
//             for (int j = 0; j < 48; j += 6) // dividing each value of R_xor_K to 8 string contaning 6 char each
//                 for (int k = j; k < j + 6; k++)
//                     s[i][j / 6] += R_xor_K[15-i][k];
//
//             s_1[i] = "";
//             for (int j = 0; j < 8; j++)
...
//
//             for (int j = 0; j < 32; j++)
//                 P_R[15-i] += s_1[i][P[j] - 1];
//
//             L_32[15-i] = R_32[15-i - 1];
//             R_32[15-i] = "";
//             R_32[15-i] = xor_add(P_R[15-i], L_32[15-i - 1]);
//
//             std::cout << "Encryption Round " << i + 1 << ":\n";
//             std::cout << "    R48: " << R48[15-i] << "\n";
//             std::cout << "    R_xor_K: " << R_xor_K[15-i] << "\n";
//             std::cout << "    P_R: " <<P_R[15-i] << "\n";
//         }
//
//         std::string RL;
//
...
//         std::cout << "Decrypted text in Windows-1251: " << encryptedText << std::endl;
//
//         std::string hexEncrypted = Bin_to_Hex(encrypted_bin);
//         std::cout << hexEncrypted << std::endl;
//
//         std::string decryptedText = binaryToWindows1251(encrypted_bin);
//         std::string originalText = removePKCS7Padding(decryptedText);
//     }
//
//     return binaryToWindows1251(encrypted_bin);
// }
//
// int main() {
//     SetConsoleCP(1251);
